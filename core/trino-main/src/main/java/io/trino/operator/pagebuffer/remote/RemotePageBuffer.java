 ...
 */
package io.trino.operator.pagebuffer.remote;
        ...
        implements PageBufferResolver, AutoCloseable
{
    ...
    public synchronized void scheduleRequest()
    {
        ...
        scheduledExecutor.schedule(() -> {
            try {
                ...
                assertNotHoldsLock(RemotePageBuffer.this);
                // should not happen, but be safe and fail the operator
                clientCallback.clientFailed(RemotePageBuffer.this, t);
            }
    ...
    private synchronized void sendGetResults()
    {
        ...
        HttpClient.HttpResponseFuture<HttpPageBufferClient.PagesResponse> resultFuture = httpClient.executeAsync(
                prepareGet()
                        ...
                        .setUri(uri).build(),
                new PageResponseHandler(dataIntegrityVerification != FeaturesConfig.DataIntegrityVerification.NONE));

        future = resultFuture;
        Futures.addCallback(resultFuture, new FutureCallback<>()
        {
            @Override
            public void onSuccess(HttpPageBufferClient.PagesResponse result)
            {
                assertNotHoldsLock(RemotePageBuffer.this);
                ...
                try {
                    ...
                    synchronized (RemotePageBuffer.this) {
                    ...
                    pagesAccepted = clientCallback.addPages(RemotePageBuffer.this, pages);
                }
                ...
                clientCallback.requestComplete(RemotePageBuffer.this);
            }
            ...
            public void onFailure(Throwable t)
            {
                ...
                if (t instanceof HttpPageBufferClient.ChecksumVerificationException) {
    ...
    @VisibleForTesting
    public synchronized void requestSucceeded(long responseSize)
    {
    ...
    {
        HttpClient.HttpResponseFuture<StatusResponseHandler.StatusResponse> resultFuture = httpClient.executeAsync(prepareDelete().setUri(location).build(), createStatusResponseHandler());
        future = resultFuture;
        Futures.addCallback(resultFuture, new FutureCallback<>()
        {
            @Override
            public void onSuccess(@Nullable StatusResponseHandler.StatusResponse result)
            {
                ...
                requestsCompleted.incrementAndGet();
                clientCallback.clientFinished(RemotePageBuffer.this);
            }
    ...
    public boolean equals(Object o)
    {
        ...
        RemotePageBuffer that = (RemotePageBuffer) o;
    ...
    public static class PageResponseHandler
            implements ResponseHandler<HttpPageBufferClient.PagesResponse, RuntimeException>
    {
        ...
        @Override
        public HttpPageBufferClient.PagesResponse handleException(Request request, Exception exception)
        {
            throw propagate(request, exception);
        }

        @Override
        public HttpPageBufferClient.PagesResponse handle(Request request, Response response)
        {
        ...
        private void verifyChecksum(long readChecksum, List<Slice> pages)
        {
            if (dataIntegrityVerificationEnabled) {
                ...
                if (readChecksum != calculatedChecksum) {
                    throw new HttpPageBufferClient.ChecksumVerificationException(format("Data corruption, read checksum: 0x%08x, calculated checksum: 0x%08x", readChecksum, calculatedChecksum));
                }
            }
            else {
                if (readChecksum != NO_CHECKSUM) {
                    throw new HttpPageBufferClient.ChecksumVerificationException(format("Expected checksum to be NO_CHECKSUM (0x%08x) but is 0x%08x", NO_CHECKSUM, readChecksum));
                }
            }
        }
